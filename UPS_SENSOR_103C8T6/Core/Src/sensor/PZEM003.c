...
#define REG_HIGH_VOLTAGE_ALARM    0x0006
#define REG_LOW_VOLTAGE_ALARM   0x0007
...
static uint32_t pzem003_last_read_tick = 0U;

static PZEM003Values_t latest_value;
...
static bool checkCRC(const uint8_t *buf, uint16_t len){
    if(len <= 2) // Sanity check
        return false;
...
void PZEM003_Task(void)
{
    switch (pzem_state)
    {
        case PZEM003_STATE_IDLE:
            {
                if ((HAL_GetTick() - pzem003_last_read_tick) < UPDATE_TIME)
                {
                    break;
                }
                ...
                pzem_state = PZEM003_STATE_SEND_REQ;
                break;
            }
            

        case PZEM003_STATE_SEND_REQ:
        {
            sendCmd8(CMD_RIR, 0x0000U, 0x0008U);
            pzem003_state_tick = HAL_GetTick();
            pzem_state = PZEM003_STATE_WAIT_TX_DONE;
            break;
        }

            

        case PZEM003_STATE_WAIT_TX_DONE:
        {
            ...
            if ((HAL_GetTick() - pzem003_state_tick) > READ_TIMEOUT)
            {
                pzem_state = PZEM003_STATE_ERROR;
                break;
            }
            ...
            {
                UART_Comm_ClearFlags(dc_id);
                pzem_state = PZEM003_STATE_START_RX;
            }
            break;
        }
        ...
        case PZEM003_STATE_START_RX:
        {
            ...
            pzem003_state_tick = HAL_GetTick();
            pzem_state = PZEM003_STATE_WAIT_RX_DONE;
            break;
        }

        case PZEM003_STATE_WAIT_RX_DONE:
        {
            if (UART_Comm_HasError(dc_id) != 0U)
            {
                pzem_state = PZEM003_STATE_ERROR;
            ...
            {
                pzem_state = PZEM003_STATE_PARSE;
            }
            break;
        }

        case PZEM003_STATE_PARSE:
        {
            // uint16_t low_word;
            // uint16_t high_word;
            ...
            latest_valid = 1U;
            new_data_ready = 1U;
            pzem003_last_read_tick = HAL_GetTick();
            pzem_state = PZEM003_STATE_IDLE;
            break;
        }

        case PZEM003_STATE_ERROR:
        {
            UART_Comm_Abort(dc_id);
            UART_Comm_ClearFlags(dc_id);
            pzem_state = PZEM003_STATE_IDLE;
            latest_valid = 0U;
            break;
